...
class EntryDetailsPage extends StatelessWidget {
          ...
          TextButton(
            onPressed: () {
               Navigator.push(context, MaterialPageRoute(builder: (_) => EntryForm(
                 user: user, cashbookId: cashbookId, initialType: data['type'], existingEntryId: entryId, existingData: data, currencySymbol: currencySymbol,
               )));
            },
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Row(children: [
                              Icon(PhosphorIcons.calendar(PhosphorIconsStyle.bold), size: 14, color: mainColor),
                              const SizedBox(width: 4),
                              Text(DateFormat('MMM d, y').format(date), style: GoogleFonts.outfit(fontSize: 12, fontWeight: FontWeight.w600, color: mainColor)),
                            ]),
                          ],
                        ),
                        ...
                        Text("${isIncome ? '+' : '-'} $currencySymbol ${(data['amount'] ?? 0).toStringAsFixed(2)}", style: GoogleFonts.outfit(fontSize: 32, fontWeight: FontWeight.w800, color: const Color(0xFF1E293B))),
                        ...
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                             _buildDetailItem("Time", DateFormat('h:mm a').format(date), PhosphorIcons.clock(PhosphorIconsStyle.fill), mainColor),
                             _buildDetailItem("Category", data['category'] ?? '-', PhosphorIcons.tag(PhosphorIconsStyle.fill), mainColor),
                             _buildDetailItem("Method", data['paymentMethod'] ?? 'Cash', PhosphorIcons.creditCard(PhosphorIconsStyle.fill), mainColor),
                          ],
                        ),
                      ],
                    ),
            ...
            // HISTORY SECTION
            Align(alignment: Alignment.centerLeft, child: Text("HISTORY", style: GoogleFonts.outfit(fontSize: 12, fontWeight: FontWeight.bold, color: const Color(0xFF4F46E5)))),
            ...
            Container(
              ...
              child: Column(
                children: history.isEmpty 
                  ? [_buildLogItem(color: const Color(0xFF4F46E5), title: "Entry Created", time: DateFormat('MMM d, h:mm a').format(date), isLast: true)]
                  : history.reversed.map((log) {
                      String title = log['action'] ?? "Update";
                      String subtext = "";
                      if (log['old'] != null && log['new'] != null) {
                        subtext = "${currencySymbol}${log['old']} -> ${currencySymbol}${log['new']}";
                      }
                      return _buildLogItem(
                        color: const Color(0xFF4F46E5), 
                        title: title, 
                        time: DateFormat('MMM d, h:mm a').format((log['date'] as Timestamp).toDate()), 
                        isLast: history.indexOf(log) == 0,
                        subtext: subtext
                      );
                    }).toList(),
              ),
  ...
  Widget _buildDetailItem(String label, String value, IconData icon, Color color) {
    return Column(
      ...
      children: [
        Text(label.toUpperCase(), style: GoogleFonts.outfit(fontSize: 10, fontWeight: FontWeight.bold, color: color)),
        const SizedBox(height: 4),
        Row(children: [
  ...
  Widget _buildLogItem({required Color color, required String title, required String time, bool isLast = false, String subtext = ""}) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Column(
          children: [
            ...
            if (!isLast) Container(width: 2, height: 40, color: Colors.grey.shade100),
          ],
        ),
        ...
        Column(
          ...
          children: [
            ...
            if (subtext.isNotEmpty) Text(subtext, style: GoogleFonts.outfit(fontSize: 12, fontWeight: FontWeight.bold, color: color)),
